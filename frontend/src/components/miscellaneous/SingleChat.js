...
import { Avatar, Box, CircularProgress, CircularProgressLabel, Flex, FormControl, IconButton, Menu, MenuButton, MenuItem, MenuList, Portal, Spinner, Text, useBreakpointValue } from "@chakra-ui/react";
...
import { mapToObject, shortendMsg } from "../../config/notificationLogics";
...
export const SingleChat = ({ fetchAgain, setFetchAgain }) => {
  ...
  const handleFileSelection = (e, fileType) => {
    ...
    for (let i = 0; i < files.length; i++) {
      if (files[i].size <= 100 * 1024 * 1024) {
        ...
        largeFiles++;
        toast.warning(`${files[i].name} exceeds the limit (100MB).`);
      }
    }

    if (largeFiles === files.length) {
      toast.error("Please upload files within the 100MB limit.");
      return;
    }
  ...
  const fecthMessages = () => {
    ...
    axios
      ...
      .catch((error) => {
        toast.error("Error", error.response ? error.response.data.message : "Something Went Wrong in fetching messages");
      })
      .finally(() => {
        setLoading(false);
  ...
  const sendMessage = (e) => {
    if (e.key === "Enter" && newMessage) {
      ...
      setNewMessage("");
      axios
        .post(`${BACKEND_URL}/api/message`, { content: newMessage, chatId: selectedChat._id }, config)
        .then(({ data }) => {
          socket.emit("new message", data.data.message);
          setMessages([...messages, data.data.message]);
        })
  ...
  const beautifyMessage = (e) => {
    if (!newMessage) {
      toast.error("Error", "Please Enter a message");
      return;
    } else {
      axios
        .post(`https://ai-helper-gules.vercel.app/api/beautify-message`, { message: newMessage })
        .then(({ data }) => {
          console.log("dattttttt", data);

          setNewMessage(data.beautified);
        })
        .catch((error) => {
          toast.error("Error", error.response ? error.response.data.message : "Something Went Wrong");
        });
  ...
  const editMessage = (msgId, updatedMsg) => {
    ...
    axios
      .put(`${BACKEND_URL}/api/message/update?messageId=${msgId}`, { messageId: msgId, content: updatedMsg }, config)
      .then(({ data }) => {
  ...
  useEffect(() => {
      ...
      if (!selectedChatCompare || selectedChatCompare._id !== newMessageReceived.chat._id) {
        ...
        setNotifications((prevNotifications) => {
          ...
          if (updatedNotifications.has(chatId)) {
            ...
            if (!notification.messages.some((msg) => msg._id === newMessageReceived._id)) {
          ...
          localStorage.setItem("unseenNotifications", JSON.stringify(mapToObject(updatedNotifications)));
    ...
    socket.on("multiple messages received", (newMessagesReceived) => {
      ...
      if (!selectedChatCompare || selectedChatCompare._id !== newMessagesReceived[0].chat._id) {
    ...
    socket.on("message updated", (messageUpdated) => {
      ...
      // Find the index of the message in your messages array
      const messageIndex = messages.findIndex((message) => message._id === messageUpdated._id);
          ...
          >
            ...
            >
              {!selectedChat.isGroupChat ? (
                <>
                  <ProfileModal selectedUser={getSenderFull(user, selectedChat.users)}>
                    <div
                      style={{
                        display: "flex",
                      }}
                    >
                      <Avatar mt={"7px"} m={1} cursor={"pointer"} name={getSender(user, selectedChat.users)} src={getSenderFull(user, selectedChat.users).dp} h={"2.5rem"} w={"2.5rem"} />
                      <div
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                      ...
                      >
                        {shortendMsg(getSender(user, selectedChat.users), maxHeadingLength)}
                        {isTyping ? (
                          <div
                            style={{
                              fontSize: "13px",
                          ...
                          >
                            typing
                            <Lottie options={getDefaultOptions(loadingDots)} width={20} height={"50%"} />
                          </div>
                        ) : (
                          <></>
                        )}
                ...
                <>
                  <UpdateGroupChatModal fetchAgain={fetchAgain} setFetchAgain={setFetchAgain} fecthMessages={fecthMessages}>
                    ...
                    >
                      <div>
                        <Avatar mt={"7px"} m={1} name={selectedChat.chatName} src={selectedChat.gdp} h={"2.5rem"} w={"2.5rem"} />
                        {shortendMsg(selectedChat.chatName.toUpperCase(), maxHeadingLength)}
                      </div>
                      {isTyping ? (
                        <div
                          style={{
            ...
            <IconButton display={{ base: "flex", md: "none" }} fontSize={15} icon={<CloseIcon />} onClick={() => setSelectedChat("")} />
          </Box>
          <Box
            display={"flex"}
            flexDir={"column"}
          ...
          >
            {loading ? (
              ...
              <div className="messages">
                <ScrollableChat messages={messages} editHandler={editMessage} deleteHandler={deleteMessage} style={{ overflowX: "hidden" }} />

                <Box w={"100%"} display={"flex"} justifyContent={"flex-end"}>
                  {isSendingMsg ? (
                    uploadProgress === 100 ? (
                      <CircularProgress isIndeterminate color="green.400" size={8} />
                    ) : (
                      <CircularProgress value={uploadProgress} color="green.400" size={8}>
                        <CircularProgressLabel zIndex={2} fontSize={8}>
                          {uploadProgress}%
            ...
            <FormControl onKeyDown={sendMessage} isRequired display={"flex"} flexDir={"column"} alignItems={"flex-start"}>
              <Box display={"flex"} position={"relative"} alignItems={"center"} w={"100%"} maxW={"100%"}>
                <Menu>
                  <MenuButton as={IconButton} icon={<AddIcon />} borderRadius={"50%"} bg={"#FFFFFF"} width={5} />
                  <Portal>
                    <MenuList>
                      <MenuItem onClick={() => handleFileInput(fileInputDocRef)} icon={<IoIosDocument size={20} color="#7F66FF" />}>
                        Document
                        <input
                          ref={fileInputDocRef}
                          type="file"
                      ...
                      <MenuItem onClick={() => handleFileInput(fileInputImgRef)} icon={<IoMdPhotos size={20} color="#007BFC" />}>
                        Photos
                        <input
                          ref={fileInputImgRef}
                          type="file"
                      ...
                      <MenuItem onClick={() => handleFileInput(fileInputVidRef)} icon={<FaFileVideo size={20} color="#6c0101" />}>
                        Videos
                        <input
                          ref={fileInputVidRef}
                          type="file"
                      ...
                      <GifModal handleGifMessage={handleGifMessage}>
                        <MenuItem icon={<AiOutlineFileGif size={20} color="#156c01" />}>Gif</MenuItem>
                      </GifModal>
                    </MenuList>
                  </Portal>
                </Menu>
                <Box flexGrow={1}>
                  <InputEmoji value={newMessage} onChange={typingHandler} placeholder="Enter message.." />
                </Box>
                <img
                  width="20"
                  height="20"
                  src="https://img.icons8.com/?size=100&id=8939&format=png&color=484848"
                  alt="beautify"
                  style={{
                    cursor: "pointer",
                    marginRight: "10px",
