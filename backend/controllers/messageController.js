...
module.exports.sendMessage = expressAsyncHandler(async (req, res) => {
    try {
        const { content, chatId, isFileInput, fileType, isGif, gif } =
            req.body;
        ...
        if (chatData.length === 0) {
            res.status(400);
            throw new Error(
                "You are not part of the group, therefore you cannot send a message"
            );
        }
    ...
    Message.find({ chat: req.query.chatId })
        ...
        .then((messages) => {
            res.status(200).json({
                success: true,
                message:
                    "All messages of a particular chat fetched successfully",
                data: messages,
            });
        })
        .catch((err) => {
...
module.exports.updateMessage = expressAsyncHandler(
    async (req, res) => {
        try {
            ...
            // Populate sender and chat fields of the message
            updatedMsg = await updatedMsg.populate(
                "sender",
                "name dp"
            );
...
module.exports.deleteMessage = expressAsyncHandler(
    async (req, res) => {
        try {
            ...
            if (
                chat &&
                chat.latestMessage &&
                chat.latestMessage.equals(message._id)
            ) {
